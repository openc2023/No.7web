...
export const PageHeaderDef: ComponentDefinition = {
  ...
  render: (p, ctx) => {
      const links = p.navLinks ? p.navLinks.split(',').map((s:string) => s.trim()) : [];
      ...
      const logoHtml = brand.type === 'image' && brand.imageUrl 
            ...
            : `<div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm" style="background-color: var(--primary); box-shadow: 0 0 15px var(--primary);"><span style="font-family: monospace;">${brand.iconClass ? brand.iconClass[0].toUpperCase() : '7'}</span></div>`;

      return `
        <div class="glass-panel w-full h-full flex ${isVertical ? 'flex-col py-6 items-center' : 'flex-row items-center justify-between px-6'}" style="background-color: rgba(9,9,11,0.8);">
            <div class="flex items-center gap-3 font-bold text-lg ${isVertical ? 'flex-col text-center' : ''}">
                ${logoHtml}
                <span style="letter-spacing: -0.025em; color: var(--text);">${brand.title}</span>
            </div>
            <nav>
                <ul class="flex ${isVertical ? 'flex-col gap-4' : 'flex-row gap-6'} text-sm font-medium opacity-80">
                    ${links.map((l: string) => `<li><a href="#" class="hover:text-purple-400 transition-colors" style="color: var(--text)">${l}</a></li>`).join('')}
                </ul>
            </nav>
        </div>
      `;
